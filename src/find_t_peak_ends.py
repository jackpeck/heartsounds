...
def find_t_peak_ends(signal, sample_rate, agreed_r_peaks, rpeaks_where_next_is_also_agreed, agreement_time_threshold = 0.1, max_disagreers = 1, root_detector = 'cwt', agree_reduce_method = Agree_reduce_method.MEDIAN):
...
def find_t_peak_ends_by_detector (signal, sample_rate, agreed_r_peaks, rpeaks_where_next_is_also_agreed):
    ...
    _, waves_peak = nk.ecg_delineate(signal, agreed_peaks_indices,
                                     sampling_rate = sample_rate, method="peak")

    _, waves_cwt = nk.ecg_delineate(signal, agreed_peaks_indices,
                                    sampling_rate=sample_rate, method="cwt")

    _, waves_dwt = nk.ecg_delineate(signal, agreed_peaks_indices,
                                    sampling_rate=sample_rate, method="dwt")
    
    rpeaks_where_next_is_also_agreed_indices = [np.argmin(np.abs(signal_times - t)) for t in rpeaks_where_next_is_also_agreed]
    
    minima_gap = find_t_peak_ends_minima_gap(signal, rpeaks_where_next_is_also_agreed_indices, agreed_peaks_indices)
